...
class SBIOnlineInference:
    ...
    def infer_scales(self, cgm_history: Sequence[float]) -> SBIInferenceResult | None:
        ...
        try:
            ...
            return SBIInferenceResult(scales={}, scales_std={}, uncertainty_score=0.0, status=f"sample_failed:{type(e).__name__}")

        med = torch.median(samples, dim=0)[0].detach().cpu().numpy()
        std = torch.std(samples, dim=0, unbiased=False).detach().cpu().numpy()
        ...
        for i, key in enumerate(self.parameter_keys):
            ...
            scales_std[key] = float(max(0.0, std[i]))

        uncertainty_score = float(np.mean(np.asarray(list(scales_std.values()) or [0.0], dtype=float)))
        return SBIInferenceResult(
            scales=scales,
