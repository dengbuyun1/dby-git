from simglucose.patient.t1dpatient import Action #CHO/Insulin
...
Observation = namedtuple("Observation", ["CGM","BG"])
...
class T1DSimEnv(object):
    ...
    def mini_step(self, action):
        ...
        IOB = self.patient.get_iob()
        COB = self.patient.get_cob()

        return CHO, insulin, BG, CGM, IOB, COB
    ...
    def step(self, action, reward_fun=risk_diff):
        ...
        for _ in range(int(self.sample_time)):
            ...
            tmp_CHO, tmp_insulin, tmp_BG, tmp_CGM, tmp_IOB, tmp_COB = self.mini_step(action)
            CHO += tmp_CHO / self.sample_time
            insulin += tmp_insulin / self.sample_time
            BG += tmp_BG / self.sample_time
            CGM += tmp_CGM / self.sample_time
            IOB += tmp_IOB / self.sample_time
            COB += tmp_COB / self.sample_time
        ...
        done = BG < -np.inf or BG > np.inf
        obs = Observation(CGM=CGM,BG=BG)
    ...
    def show_history(self):
        ...
        df["IOB"] = pd.Series(self.iob_hist)
        df["COB"] = pd.Series(self.cob_hist)
