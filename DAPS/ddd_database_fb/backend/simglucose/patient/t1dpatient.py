...
Observation = namedtuple("observation", ["Gsub"])  # 皮下glucose

PATIENT_PARA_FILE = pkg_resources.resource_filename(
    "simglucose", "params/vpatient_params.csv"
)
CUSTOM_PARA_FILE = pkg_resources.resource_filename(
    "simglucose", "params/custom_vpatient_params.csv"
)


def _load_all_params():
    """Helper to load default and custom params"""
    df_default = pd.read_csv(PATIENT_PARA_FILE)
    try:
        if os.path.exists(CUSTOM_PARA_FILE):
            df_custom = pd.read_csv(CUSTOM_PARA_FILE)
            # Ensure columns match before concatenating
            if not df_custom.empty:
                return pd.concat([df_default, df_custom], ignore_index=True)
    except Exception as e:
        logger.warning(f"Failed to load custom params: {e}")
    return df_default
...
class T1DPatient(Patient):
    ...
    def withID(cls, patient_id, **kwargs):
        ...
        """
        patient_params = _load_all_params()
    ...
    def withName(cls, name, **kwargs):
        ...
        if params.empty:
            raise ValueError(f"Patient {name} not found in database")
        return cls(params, **kwargs)
    ...
    def get_iob(self):
        ...
        params = self._params
        total_insulin = (
            (state[5] + state[9] + state[10] + state[11]) * params.BW / 6000.0
        )  # glucose insulin + liver insulin + subcutaneous insulin fast + subcutaneous insulin slow
        ss_insulin = (
            (params.Ipb + params.Ilb + params.isc1ss + params.isc2ss)
            * params.BW
            / 6000.0
        )
        iob = total_insulin - ss_insulin
        return max(iob, 0)  # insulin on board (unit: U/min)

    def get_cob(self):
        state = self.state
        params = self._params
        # state[0], state[1], state[2] are in mg (milligrams)
        total_carb = (
            state[0] + state[1] + state[2]
        )  # mg: stomach solid + stomach liquid + intestine
        # 返回 carbs on board（单位：克）
        cob_g = total_carb / 1000.0  # mg -> g
        return cob_g
